...
if __name__ == "__main__":
    args = ArgumentParser()
    args.add_argument('-d', '--dataset-path', help='Absolute path of the csv dataset to load', required=False)
    args.add_argument('-u', '--n-users', help='Number of users present in the dataset', type=int, required=False)
    args.add_argument('-m', '--n-movies', help='Number of movies present in the dataset', type=int, required=False)
    args.add_argument('-i', '--n-iterations', help='Number of iterations to run ALS algorithm for', type=int, default=1000)
    args.add_argument('-nn', '--non-negative', help='Setting this will solve least-squares with nonnegativity constraints', action='store_true', default=False)
    ...
    try:
        ...
        # %5 test size
        test_set_size = dataset.n_ratings//10
    ...
    print("Running ALS..")
    model = ALS.train(ratings, rank, args.n_iterations, lambda_=0.0, nonnegative=args.non_negative, seed=SEED)
