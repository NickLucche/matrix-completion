...
class ALS:
    ...
    def fit(self,
        ...
        latest_theta = np.ones(theta_dim).reshape(-1, 1) * np.inf
        theta = -latest_theta
        counter = 0
        ...
        while norm_grad_u > eps_g and np.linalg.norm(
            ...
            grad_v = _compute_gradient_vectorized(self.M.T,
                                                  np.asfortranarray(self.X),
                                                  self.v, self.u)

            # TODO: explaing contiguity and trade-off with memory
            self.v = _compute_minimizer(self.u.T, self.M.T,
                                        np.asfortranarray(self.X))

            theta = np.vstack([self.u, self.v]).reshape(-1, 1)

            # compute gradient of u after v was updated (grad_v is zero here)
            # v_hat = self.M * self.v.T computed inside
            grad_u = _compute_gradient_vectorized(self.M, self.X.T, self.u,
                                                  self.v)
        ...
        print('-' * 80)
        print(
            f"Estimated average iteration runtime: {(end_time - start_time) / counter}s"
        )
        self.register_stats(end_time - start_time, counter, grad_u,
                            np.linalg.norm(theta - latest_theta))
        print(
            f"Estimated average function evaluation times: {self.stats['avg_fun_eval_time']}"
        )
    ...
    def log_stats(self, norm_grad_u, grad_v, iteration):
        ...
        print(
            f'[{iteration}]\t\t{fun_eval}\t\t{norm_grad_u}\t\t{np.linalg.norm(grad_v)}'
        )
    ...
    def register_stats(self, runtime: float, n_iters: int,
                       final_grad: np.ndarray, theta_diff: float):
        self.stats['avg_iter_time'] = runtime / n_iters
        self.stats['total_convergence_time'] = runtime
        self.stats['avg_fun_eval_time'] = sum(self.fun_eval_times) / len(
            self.fun_eval_times)
...
@njit(parallel=False)
def _function_eval_numba(X: np.ndarray, u: np.ndarray, v: np.ndarray,
                         M: np.ndarray):
    rows_sum = np.zeros(X.shape[1], dtype=u.dtype).reshape(-1, 1)
    for i in range(X.shape[0]):
        rows_sum[i] = (((u[i].item() * v) * M[i].reshape(-1, 1) -
                        X[i].reshape(-1, 1))**2).sum()
    return rows_sum.sum()


@njit(parallel=False)
def _compute_minimizer(z: np.ndarray, M: np.ndarray,
...
def _compute_gradient_vectorized(M: np.ndarray, X: np.ndarray, z: np.ndarray,
    ...
    for i in prange(z.shape[0]):
        ...
        grad_z[i] = hat_vector @ (
            z[i] * y - X[:, i].reshape(-1, 1).astype(z.dtype)).astype(z.dtype)
...
class ALSSparse:
    def __init__(self, u: np.ndarray, v: np.ndarray,
                 dataset: sparse.csr.csr_matrix) -> None:
    ...
    def fit(self,
        ...
        while norm_grad_u > eps_g and np.linalg.norm(
            ...
            sparse_v = sparse.lil_matrix((self.v.shape[0], self.v.shape[0]),
                                         dtype=self.v.dtype)
            ...
            # *** minimize wrt v ***
            # this time u_hat will have j-th *column* corresponding to \hat{u}_j since u represents users
            sparse_u = sparse.lil_matrix((self.u.shape[0], self.u.shape[0]),
                                         dtype=self.u.dtype)
            ...
            grad_v = _compute_sparse_gradient(u_hat.T, self.X_T, self.v,
                                              self.u)
    ...
    def function_eval(self):
        ...
        differences, _ = _compute_sparse_difference_matrix(
            sparse_X_tuple, self.u, self.v)
...
@njit(parallel=False)
def _compute_sparse_difference_matrix(X_sparse_repr: tuple, z: np.ndarray,
                                      y: np.ndarray):
...
if __name__ == "__main__":
    ...
    u = np.random.randn(10).astype(np.float32).reshape(-1, 1)
    v = np.random.randn(3).astype(np.float32).reshape(-1, 1)
    X_float = sparse.random(10, 3, density=.4,
                            dtype=np.float32).power(2).tocsr() * 10
